...
2. envelope の `success` を確認後、`data` 配列を取り出す（`--machine` の `data` は平坦な配列。`data.candlestick` は存在しない）
3. 各行は `{open, high, low, close, vol, timestamp}` オブジェクト（CLI が数値正規化済み、timestamp はミリ秒 UNIX/UTC）
4. 配列は昇順（古い順）。先頭が最古、末尾が最新
