...
3. **申告要否・税額・所得区分を判定しない**。20 万円ルールも同様に判定しない
   ...
   手元に免責文が無ければ `bitbank tax pnl --year=<年> --format=json --machine` で
   取得する（`--year` だけで実行でき、参考損益がブロックされていても `disclaimers`
   は常に出力される）。年分が不明・認証未設定などで実行できない間は、記憶から
   言い換えて答えず、不足している入力を確認する
