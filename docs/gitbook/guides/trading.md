...
read-only profile → paper → dry-run → 本番という段階フローを [レシピ集](recipes.md#bot-運用の段階フロー) と、リポジトリの [botter 運用 Runbook](https://github.com/tjackiet/bitbank-lab-cli/blob/main/docs/botter-runbook.md) にまとめています。監視には読み取り専用キー、取引には取引用キーを別プロファイルにして、誤爆の被害を局所化してください（[API キーの設定](../getting-started/api-keys.md) を参照）。
