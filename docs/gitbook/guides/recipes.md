...
実際の資金で 24/7 取引を回す前に通すべき手順は、**read-only → paper → dry-run → 本番**の段階フローとして整理されています。各段階には「次へ進む条件」があり、影響範囲を一段ずつ広げます。詳しくはリポジトリの [botter 運用 Runbook](https://github.com/tjackiet/bitbank-lab-cli/blob/main/docs/botter-runbook.md) を参照してください。取引の安全ガードそのものは [取引と安全ガード](trading.md) にまとめています。
