...
### 残っている実データの用途（1 本入手して一部消化）

仕様側は上記で解決し、**2026-07-27 に 2025 年分の実 CSV を 1 本入手**した（実機確認 #10）。
当初挙げていた 3 点の消化状況:

1. ~~**実ファイルの物理様式**~~ → **完了**（実機確認 #10）。現物と 4 点違っていた。
   特に見出しの `（円）` は **PR #16 の修正が無ければファイル全体を拒否していた**
2. **突合が実際に通るか** → **未消化**。突合には報告書と**同じ口座の API 履歴**が要る。
   入手したのは CSV 単体なので、様式検証（パーサ疎通・列対応・未知列ゼロ）までしか進めない。
   当該口座で `bitbank tax verify-report --year=2025 --margin-csv=...` を回してもらうのが次の一手
