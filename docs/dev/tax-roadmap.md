...
2026-07-24 のヒアリングで B 層の主ターゲットを**「中口マニュアル勢」**（高頻度ではないが
積極的に取引する個人、年間約定 3 万件目安）に絞り込んだ。超大口 HFT はツール自作勢が
多く非ターゲット（→「ヒアリング結果の反映」）。
...
## ヒアリング結果の反映（2026-07-24 受領・大口営業部）

大口営業部（トップ 500 を Tier1=トップ 20 / Tier2=トップ 50 / Tier3=トップ 500 で管理、
直接接点は約 60 ユーザー・下位 Tier ほど入れ替わりが激しくカバー率低下）経由の
規模感・実態データ。外部入力待ちだった「B 層の規模感」はこれで解消。

### ターゲットの絞り込み

- **主ターゲットは「中口マニュアル勢」**（高頻度ではないが積極的に取引する個人）に確定
  - リテラシーの高い超大口（HFT/bot）はツール自作勢が存在し（自作ツールを他ユーザーに
    使わせている例もある）、提供ツールを使わない可能性が高い → 非ターゲット
  - bitbank 単独取引の個人は**年間取引報告書を確定申告に転写すれば済む**ケースが多く、
    複雑な計算は不要 → 他取引所と掛け持ちするユーザーへの整合データ出力
    （= 正規化、A 層価値）の比重が上がる
  - 税理士丸投げ層も多いが税理士側が計算できないケースもある → 「整形は bitbank 側、
    最終計算はユーザー/税理士」という ADR-004 / B 案の役割分担の裏付け
  - 「20 万円超の利益が出たらどうすればいいか」への AI サポートは有効
    （Week 8-9 の C 層 MCP 対話フローの価値を裏付け）

### 規模感（性能目標の確定）

- 超大口 HFT: トップ 12 合計で月間約 100 万注文（1 人あたり 5〜10 万注文/月）。
  約定率は 1〜3% 程度のため、税務計算上のレコード数は**年間約 1 万件が目安**
- **性能目標は「年間約定 3 万件」を快適に処理できること**。月 1 万件処理で大多数の
  大口をカバー、10 万件でほぼ全大口だが、そこまでの超大口は非ターゲットと割り切る
- 既存 SaaS（Cryptact 等）は数万件レコードでエラーが返る報告あり（高頻度層ほど
  使えない）→ bitbank 側整形の価値の裏付け。ただし利用実態は再調査（下記）
- 法人アカウントは大口の 2〜3 割で、個人のまま高頻度取引する大口が多数
  （法人化しない理由: 帳簿管理の煩雑さ・税理士依頼の手間）

### 設計への含意

- **年間取引報告書フォーマットへの出力対応を Week 4 の検討対象に追加**。検証アンカー 1
  と同じ様式を出力仕様側でも使う案（bitbank 単独ユーザーは転写で完結する、の延長線）。
  Cryptact/Gtax 直接出力 vs 汎用フォーマットの決定と合わせて判断する
- **サブアカウント横断集計はスコープ外**。稼働 20〜30 人、システム的に横断集計は
  不可で CX が Slack 経由の手動 CSV で対応中。サブアカウントの扱い自体が今後変更され
  得るため、現時点で作り込むと損をするリスクがある
- **VIP プログラムのキャッシュバック報酬はスコープ外**（超大口のみが対象）。
  API 外課税イベントの棚卸し（実機確認 #6 / Week 4 判断）にこの整理を反映する
- **信用取引はツール対応の優先度低・ただし履歴取り込みは対応**（現物より利用者が
  少ない）。実機 #2 の解読結果・決済 `profit_loss` ネット値採用・分解明細併記（P-06）
  の方針は維持し、優先順位のみ下げる
- リベート（マイナス手数料）は別カラム管理で取引利益と分離計算可能 →
  P-04（受取時収入計上の単一標準）と整合

### 要確認（既存記述との突合）

- ヒアリングでは「リベートは**暗号資産ベースでの調整が伴う**」との言及があった。
  実機確認 #3 では検証口座のリベートは JPY（quote）建てのみ・base 通貨建て手数料 0 件。
  どの経路で暗号資産建ての調整が発生するのか（過去年データ・特定ペア・プログラム依存か）
  を確認する。段階 3 の負の手数料設計（所得区分・簿価影響）に影響し得る

### アクションアイテム

- [ ] ターゲットを「中口マニュアル勢」に絞った企画の具体化（Week 4 正規化仕様と接続）
- [ ] フジモン（藤本さん）と税務ツールの協業可能性について対話（人間側）
- [ ] Cryptact の実際の利用実態（使われているか否か）の再調査（人間側）
- [ ] 年間取引報告書フォーマットへの出力対応を出力仕様として検討（Week 4・CLI 側）
- [ ] リベートの「暗号資産ベース調整」の実態確認（上記「要確認」）
...
B 層の規模感・法人比率・現状ワークフローは 2026-07-24 に大口営業部経由で受領済み
（→「ヒアリング結果の反映」）。残るは A 層の SaaS フォーマット要件と
Cryptact 利用実態の再調査。
...
- ~~B 層ヒアリング~~ → **受領**（2026-07-24、大口営業部経由。
  →「ヒアリング結果の反映」）。派生の残タスク:
  1. Cryptact の利用実態（使われているか否か）の再調査（人間側）
  2. フジモン（藤本さん）との協業可能性の対話（人間側）
  3. リベートの「暗号資産ベース調整」の実態確認（実機 #3 の JPY 建てのみ観測と
     突合が必要）
...
- Week 2 の生データ取得網羅（①〜⑥）は完了（PR #5 まで merge 済み）
- Week 3 の B 層ヒアリングは 2026-07-24 に受領・反映済み（→「ヒアリング結果の反映」）。
  ターゲットは「中口マニュアル勢」・性能目標は年間約定 3 万件に確定
- 次は Week 4（正規化仕様）へ。検討項目に**年間取引報告書フォーマットへの出力対応**が
  追加された（Cryptact/Gtax 直接出力 vs 汎用フォーマットの決定と合わせて判断）。
  並行してトラック 3（投資オンボーディング/ツール最適化）の設計着手も可
- 人間側タスク: A 層ヒアリング・Cryptact 利用実態再調査・フジモン氏協業対話・
  リベート「暗号資産ベース調整」の実態確認
