...
| Week 7 | 損益計算を CLI Skill に統合、B 層（VIP/MM 含む）先行公開。**検証アンカーとの突合開始** | `bitbank tax verify-report`（アンカー 1 の自動突合）/ `skills/tax-report` Step 3〜4 | **アンカー 1 は着手済み**（アンカー 2 の互換モードは未着手） |
...
**クリティカルパスは実装ではなく外部入力**（2026-07-26 更新）: 実装側は Week 2 完了 +
数値基盤（Week 6 相当）が先行済みで見通しが立っている。一方、**付録B の【方針】の税務監修**と
**免責文言の法務レビュー**は未着手で、これらが 11 月の実データ検証より前に片付いていないと、
検証で出た数字の意味が確定しない。

ただし監修の対象は**19 件全部ではない**。付録B に監修要否の列が入り、
...
| # | アンカー | 位置づけ | 根拠 |
|---|---|---|---|
| 1 | bitbank 公式の年間取引報告書 | **必須** | 取引所自身が発行する当年フローの全量。取得コストはユーザーの CSV ダウンロード 1 回で、差分の意味も明確（購入・売却の差 = 販売所ぶん）。`bitbank tax verify-report` で自動突合できる |
...
- **税制調査**: 完了（2026-07-23 受領、[tax-research.md](tax-research.md) v2）。
  ...
  2. 付録 B（【方針】台帳 **P-01〜P-19**）の税務監修。**★11 件を先行**、△5 件は補足、
     −3 件（P-02 / P-15 / P-17）は技術判断のため対象外
