...
| # | アンカー | 位置づけ | 根拠 |
|---|---|---|---|
| 1 | bitbank 公式の年間取引報告書（**現物・信用の 2 本**） | **必須** | 取引所自身が発行する当年フローの全量。取得コストはユーザーの CSV ダウンロードで、差分の意味も明確（現物の購入・売却の差 = 販売所ぶん）。`bitbank tax verify-report --csv / --margin-csv` で自動突合できる |
