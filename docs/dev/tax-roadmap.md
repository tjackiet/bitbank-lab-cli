...
上記で仕様側は解決したので、**実 CSV は「仕様判断のブロッカー」ではなくなった**。
それでも 1 本欲しい理由は次の 3 つで、いずれも仕様書では担保できない:

1. **実ファイルの物理様式** — BOM の有無、メタ行（氏名・発行者）の位置、実際のヘッダ文字列
   （`（円）` がどちらか）。現物は実ファイルで確認済みだが信用は未確認
2. **突合が実際に通るか** — 信用の突合コードは一度も実データで走っていない
3. **手数料ウィンドウの疑問**（上記）の実地検出

**年をまたぐ建玉である必要はもう無い**（それが要る理由だった手数料基準が解決したため）。
信用取引の履歴がある口座なら誰のものでもよい。
