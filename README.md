...
bitbank API を叩く薄い CLI。Public（9）/ Private（13）/ Trade（6、ドライランデフォルト）/ Paper（9、ペーパートレード）/ Profile（5、API キー切替）/ WebSocket（2、`stream` / `watch`）の合計 44 コマンド。
...
自然言語で CLI を操作するための Skill を 12 本同梱。リポジトリを Claude Code / Cursor で開けば自動でトリガーされます。
...
内訳は分析系（7）/ 取引系（1、paper-trade）/ ユーティリティ（2）/ Recipe（2）。詳細は [Agent Skills](#agent-skills) を参照。
