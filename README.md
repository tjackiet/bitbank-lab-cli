...
| `paper assets` | 仮想残高を表示 | `paper assets` |
| `paper create-order` | 成行で即時 fill（last 価格） | `paper create-order --pair=btc_jpy --side=buy --type=market --amount=0.001` |
| `paper trade-history` | 仮想約定履歴 | `paper trade-history` |
| `paper reset` | 仮想口座をリセット（`--confirm` 必須） | `paper reset --confirm` |

> MVP は成行（`--type=market`）のみ。指値・ストップ・OCO は未対応です。手数料は bitbank 公称テイカー手数料（0.12%）を JPY 建てで差し引きます。スリッページは入っていません（last 即 fill）。
