...
- **1 ファイル 100 行は目安**。数えるのは**コード行**で、空行と「行全体がコメントの行」は
  含まない（chaos `x04`）。検出したいのは責務の肥大であってコメントの厚さではないため、
  **説明コメントは何行書いても上限に影響しない**。超えたら設計を見直す（責務が
  広がっていないか、リトライ・パース・整形などが混ざっていないか）。どうしても超過に
  妥当な理由がある場合は、ファイル冒頭にコメントで理由を書く
